...
def num_to_word_hindi(txt):
    txt = str(txt).replace(",", "").rstrip(".")
    if txt.isnumeric():
        if len(txt) == 10:
            return " ".join(list(map(lambda x: num_to_word(x, lang='hi', separator=" "), list(txt))))
        if 1100 <= int(txt) <= 1999:
            return num_to_word(txt[:2], lang='hi', separator=" ") + " सौ " + num_to_word(txt[2:], lang='hi', separator=" ")
        return num_to_word(txt, lang='hi', separator=" ")
    elif '-' in txt:
        return " ".join(list(map(lambda x: num_to_word(x, lang='hi', separator=" ") , txt.split("-"))))
    elif '.' in txt:
        if txt.count('.') > 1:
            return " ".join(list(map(lambda x: num_to_word(x, lang='hi', separator=" ") , txt.split("."))))
        else:
            pre, post = txt.split(".")
            answer = [num_to_word(pre, lang='hi', separator=" ")]
            answer.append("दशमलव")
            for n in post:
                answer.append(num_to_word(n, lang='hi', separator=" "))
        return " ".join(answer)
    elif ':' in txt:
        return " ".join(list(map(lambda x: num_to_word(x, lang='hi', separator=" ") , txt.split(":"))))
    else:
